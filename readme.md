...
#### Intégration bureau Linux

Après téléchargement de l'AppImage :

```bash
chmod +x Lygodactylus-*-linux-*.AppImage
npm run build:linux   # ou placez l'AppImage dans release/
./scripts/install-linux-desktop.sh ./release
```

Appuyez sur **Super** et cherchez « Lygodactylus » dans le lanceur d'applications.
...
Pour construire un installeur local : `npm run build` (plateforme courante) ou `npm run build:linux` sur Linux.
