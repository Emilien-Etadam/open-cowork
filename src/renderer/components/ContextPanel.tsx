...
export function ContextPanel() {
  ...
  if (contextPanelCollapsed) {
    return (
      <div className="w-9 bg-background border-l border-border-muted flex items-start justify-center pt-3">
        <button
  ...
  return (
    <div className="w-[16.2rem] bg-background border-l border-border-muted flex flex-col overflow-hidden text-sm">
      {/* Header */}
      ...
      {activeSession && contextUsage && (
        <div className="px-4 py-2.5 border-b border-border-muted space-y-1.5">
          ...
          {contextUsage.reservedOutput > 0 && (
            <p className="text-xs text-text-muted">
              {t('context.contextUsageDetail', {
                input: formatTokenCount(contextUsage.inputTokens),
                output: formatTokenCount(contextUsage.reservedOutput),
