...
**Open Cowork** is a desktop AI agent app (Electron) for Windows and macOS. It manages your files, generates documents via **Skills** (PPTX, DOCX, XLSX, PDF), connects to tools through **MCP**, isolates commands in a **sandbox** (WSL2 / Lima), and can be controlled remotely via **Feishu** or **Slack**.
...
- **Sandbox toggle** — enable/disable restored in Settings → Sandbox
- **pi-coding-agent 0.73.1** — recent agent migration + context compaction fixes
...
3. Choose a **workspace folder** — the agent will only work inside that directory.
4. Send a prompt, for example: _"Read report.csv and create a 5-slide PowerPoint summary."_

For full documentation (API providers, sandbox setup, skills, MCP), see the [upstream README](https://github.com/OpenCoworkAI/open-cowork/blob/main/readme.md).
