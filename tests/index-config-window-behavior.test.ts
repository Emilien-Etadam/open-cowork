...
describe('Main process window/config behavior', () => {
  ...
  it('session.start blocked by active set emits structured error without forcing config.status', () => {
    ...
    expect(sessionStartGuard).toContain("event.type === 'session.start'");
    expect(sessionStartGuard).toContain("event.type === 'session.compact'");
    expect(sessionStartGuard).toContain("event.type === 'session.handoff'");
